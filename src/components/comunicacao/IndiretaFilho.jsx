...
const IndiretaFilho = (props) => {
  ...
  const [isNerd, setIsNerd] = useState(""); // Estado para saber se é nerd
  ...
  // Função para tratar mudanças no campo de "é nerd?"
  const handleNerdChange = (event) => {
    setIsNerd(event.target.value);
  };
  ...
  const handleAddData = () => {
    if (nameUsuario && age && !ageError && isNerd) {
      const newData = { nameUsuario, age, isNerd };
  ...
  return (
    <div>
      ...
      <br />
      <label>
        <span style={{ marginRight: "10px" }}>É nerd?</span>
        <select value={isNerd} onChange={handleNerdChange}>
      ...
      <div className="App">
        <button onClick={showAddModalFunc}>Fornecer Informações</button>
        <Modal
          show={showAddModal}
              ...
              <tbody>
                ...
                {filteredData.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? "Par" : "Impar"}>
                    ...
                    <td>{item.isNerd === "Sim" ? "Sim" : "Não"}</td>
                    <td>
                      {/* Botões para excluir e editar um item */}
